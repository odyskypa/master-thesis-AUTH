...
def xml_to_str(final_xml,flag=False):
    """
    ...
    if final_xml == -1:
        ...
        if flag == False:
            ...
            final_ast = final_xml.splitlines()
...
def get_lowest_common_ancestor_additions_deletions(root, nodes, return_furthest_for_one_node = False):
    ...
    if len(nodes) == 0:
        ...
        if return_furthest_for_one_node:
            ...
            while len(ancestors) > 0:
                ancestor = ancestors.pop()
                if len([child for child in ancestor]) > 2 and len([child for child in previous_ancestor]) != 0:
                    return previous_ancestor
                previous_ancestor = ancestor
        return previous_ancestor
    else:
...
def close_xml_tags(code_ast):
    """
    Closes unclosed tags of asts for pair <MethodDeclaration> - </MethodDeclaration> by checking if the
    number of tags are equal or there is one closing tag missing.
    
    param : code_ast: The ast we are checking for missing closing tags
    
    return: code_ast: The fixes ast with same number of opening and closing tags for <MethodDeclaration>
    """
    if type(code_ast) == str:
        open_tag_count = code_ast.count("<MethodDeclaration>")
        closed_tag_count = code_ast.count("</MethodDeclaration>")
        
        if open_tag_count == closed_tag_count + 1 :
            code_ast = code_ast + "</MethodDeclaration>"
